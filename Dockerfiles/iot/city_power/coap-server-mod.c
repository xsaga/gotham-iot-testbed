...
/* Updated to simulate IoT device reporting city power consumption for three
 * zones based on the https://archive.ics.uci.edu/ml/datasets/Power+consumption+of+Tetouan+city
 * dataset.
 */
...
#define DATASETFNAME "TetuanCityPowerConsumption.csv"
#define DATASETBUFFSIZE 200
#define DATASETCOLSIZE 50
...
#define HUMIDITY 3
#define WINDSPEED 4
#define GENERALDIFFUSEFLOWS 5
#define DIFFUSEFLOWS 6
#define ZONE1POWER 7
#define ZONE2POWER 8
#define ZONE3POWER 9
...
#define INDEX "-----------------------City Power Consumption-----------------------\n" \
...
void dataset_read(char *col_data, FILE *ifp, char *delim, int col_num) {
  ...
  if(fgets(buffer, DATASETBUFFSIZE, ifp) == NULL) {
    ...
    if(feof(ifp)){
      ...
      printf("Error reading dataset: fgets returned NULL but not in EOF.");
    }
  }
...
/* handle dataset datetime column */
FILE *fp_datetime;

static void
hnd_get_datetime(coap_resource_t *resource,
                   coap_session_t *session,
                   const coap_pdu_t *request,
                   const coap_string_t *query,
                   coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_datetime, FIELDSEPARATOR, DATETIME);
...
/* handle dataset temperature column */
FILE *fp_temperature;
...
hnd_get_temperature(coap_resource_t *resource,
                    ...
                    coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_temperature, FIELDSEPARATOR, TEMPERATURE);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
/* handle dataset humidity column */
FILE *fp_humidity;

static void
hnd_get_humidity(coap_resource_t *resource,
                 coap_session_t *session,
                 const coap_pdu_t *request,
                 const coap_string_t *query,
                 coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_humidity, FIELDSEPARATOR, HUMIDITY);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
/* handle dataset wind speed column */
FILE *fp_windspeed;
...
/* handle dataset general diffuse flows column */
FILE *fp_generaldiffuseflows;

static void
hnd_get_generaldiffuseflows(coap_resource_t *resource,
                   coap_session_t *session,
                   const coap_pdu_t *request,
                   const coap_string_t *query,
                   coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_generaldiffuseflows, FIELDSEPARATOR, GENERALDIFFUSEFLOWS);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
/* handle dataset diffuse flows column */
FILE *fp_diffuseflows;

static void
hnd_get_diffuseflows(coap_resource_t *resource,
                     coap_session_t *session,
                     const coap_pdu_t *request,
                     const coap_string_t *query,
                     coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_diffuseflows, FIELDSEPARATOR, DIFFUSEFLOWS);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
/* handle dataset Zone 1 Power Consumption column */
FILE *fp_zone1power;

static void
hnd_get_zone1power(coap_resource_t *resource,
                   coap_session_t *session,
                   const coap_pdu_t *request,
                   const coap_string_t *query,
                   coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_zone1power, FIELDSEPARATOR, ZONE1POWER);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
// handle dataset Zone 2 Power Consumption column
FILE *fp_zone2power;

static void
hnd_get_zone2power(coap_resource_t *resource,
                   coap_session_t *session,
                   const coap_pdu_t *request,
                   const coap_string_t *query,
                   coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_zone2power, FIELDSEPARATOR, ZONE2POWER);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
/* handle dataset Zone 3 Power Consumption column */
FILE *fp_zone3power;

static void
hnd_get_zone3power(coap_resource_t *resource,
                   coap_session_t *session,
                   const coap_pdu_t *request,
                   const coap_string_t *query,
                   coap_pdu_t *response) {
  char response_data[DATASETCOLSIZE];
  dataset_read(response_data, fp_zone3power, FIELDSEPARATOR, ZONE3POWER);
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
  coap_add_data_large_response(resource, session, request, response,
                               query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                               strlen(response_data),
...
init_resources(coap_context_t *ctx) {
  ...
  r = coap_resource_init(coap_make_str_const("datetime"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_datetime);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"DateTime M/D/YYYY H:MM\""), 0);
  coap_add_resource(ctx, r);
  ...
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Temperature\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("humidity"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_humidity);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Humidity\""), 0);
  coap_add_resource(ctx, r);
  ...
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Wind Speed\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("generaldiffuseflows"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_generaldiffuseflows);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"general diffuse flows\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("diffuseflows"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_diffuseflows);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"diffuse flows\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("zone1power"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_zone1power);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Zone 1 Power Consumption (kWh)\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("zone2power"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_zone2power);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Zone 2 Power Consumption (kWh)\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("zone3power"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_zone3power);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Zone 3 Power Consumption (kWh)\""), 0);
  coap_add_resource(ctx, r);
}
...
main(int argc, char **argv) {
  ...
  fp_datetime = fopen(DATASETFNAME, "r");
  if(fp_datetime == NULL){
    printf("Can't open dataset (datetime).\n");
    return 1;
  } else {
    printf("Opened dataset (datetime).\n");
  }
  ...
  if(fp_temperature == NULL){
    ...
    printf("Opened dataset (temperature).\n");
  }
  ...
  if(fp_humidity == NULL){
    ...
    printf("Opened dataset (humidity).\n");
  }
  ...
  if(fp_generaldiffuseflows == NULL){
    ...
    printf("Opened dataset (generaldiffuseflows).\n");
  }
  ...
  if(fp_diffuseflows == NULL){
    ...
    printf("Opened dataset (diffuseflows).\n");
  }
  ...
  if(fp_zone1power == NULL){
    ...
    printf("Opened dataset (zone1power).\n");
  }
  ...
  if(fp_zone2power == NULL){
    ...
    printf("Opened dataset (zone2power).\n");
  }
  ...
  if(fp_zone3power == NULL){
    ...
    printf("Opened dataset (zone3power).\n");
  }
  ...
  fclose(fp_datetime);
  printf("Closed dataset (datetime).\n");

  fclose(fp_temperature);
  printf("Closed dataset (temperature).\n");

  fclose(fp_humidity);
  printf("Closed dataset (humidity).\n");

  fclose(fp_windspeed);
  printf("Closed dataset (windspeed).\n");

  fclose(fp_generaldiffuseflows);
  printf("Closed dataset (generaldiffuseflows).\n");

  fclose(fp_diffuseflows);
  printf("Closed dataset (diffuseflows).\n");

  fclose(fp_zone1power);
  printf("Closed dataset (zone1power).\n");

  fclose(fp_zone2power);
  printf("Closed dataset (zone2power).\n");

  fclose(fp_zone3power);
  printf("Closed dataset (zone3power).\n");
