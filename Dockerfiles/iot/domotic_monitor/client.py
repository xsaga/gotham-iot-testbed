...
def telemetry(dataset_fname, sleep_t, sleep_t_sd, event, die_event, mqtt_topic, broker_addr, mqtt_qos, mqtt_tls, mqtt_cacert, mqtt_tls_insecure):
    ...
    while not die_event.is_set():
        if event.is_set():
            ...
            try:
                ...
                publish.single(topic=mqtt_topic, payload=payload, qos=mqtt_qos, hostname=broker_addr, port=port, tls=tls_arg.copy() if tls_arg else None)
